...
import { applyPatchRecord } from "../../../packages/patches/src/patch-store.ts";
...
if (!sessionId || !artifactId) {
  console.error("Usage: npm.cmd run patch:apply -- --session-id <id> --artifact-id <artifact-id> [--yes]");
  console.error("Default mode is dry-run. Add --yes to write files.");
  process.exit(1);
}

try {
  const result = await applyPatchRecord({
